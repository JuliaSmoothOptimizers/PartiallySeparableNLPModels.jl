...
function create_Rosenbrock_JuMP_Model2(n::Int)
  m = Model()
  @variable(m, x[1:n])
  @NLobjective(m, Min, sum(100 * (x[j - 1]^2 - x[j])^2 + (x[j - 1])^2 for j = 2:n)) #rosenbrock function
  evaluator = JuMP.NLPEvaluator(m)
  MathOptInterface.initialize(evaluator, [:ExprGraph, :Hess])
  obj = MathOptInterface.objective_expr(evaluator)
  vec_var = JuMP.all_variables(m)
...
SPS_Float32 =
  PartiallySeparableNLPModels.deduct_partially_separable_structure(comp_ext2, n, Float32)
...
@testset "test du status de convexité des fonctions éléments" begin
  f = (
    x -> CalculusTreeTools.get_convexity_wrapper(
      PartiallySeparableNLPModels.get_convexity_status(x),
    )
  )
  ...
  res = []
  for i = 1:length(PartiallySeparableNLPModels.get_structure(SPS3))
    if i % 2 == 0
