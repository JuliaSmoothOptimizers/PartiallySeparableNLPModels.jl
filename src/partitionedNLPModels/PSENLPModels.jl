...
"""
mutable struct PSENLPModel{G, P, T, S, M <: AbstractNLPModel{T, S}, Meta <: AbstractNLPModelMeta{T, S},} <: AbstractPQNNLPModel{T,S}
  nlp::M
  meta::Meta
...
function PSENLPModel(nlp::SupportedNLPModel)
  n = nlp.meta.nvar
  x0 = nlp.meta.x0
  ex = get_expression_tree(nlp)
  T = eltype(x0)

  (n, N, vec_elt_fun, M, vec_elt_complete_expr_tree, element_expr_tree_table, index_element_tree, vec_compiled_element_gradients, x, v, s, pg, pv, py, ps, phv, pB, fx, name) = partially_separable_structure(ex, n; name=:pse, x0)
  P = typeof(pB)
  ...
  counters = NLPModels.Counters()
  S = typeof(x)
  plsenlp = PSENLPModel{ExpressionTreeForge.Complete_expr_tree, P, T, S, Model, Meta}(nlp, meta, counters, n, N, vec_elt_fun, M, vec_elt_complete_expr_tree, element_expr_tree_table, index_element_tree, vec_compiled_element_gradients, x, v, s, pg, pv, py, ps, phv, pB, fx, name)
  return plsenlp
end
