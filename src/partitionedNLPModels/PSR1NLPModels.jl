...
"""
    PSR1NLPModel{G, P, T, S, M <: AbstractNLPModel{T, S}, Meta <: AbstractNLPModelMeta{T, S},} <: AbstractPQNNLPModel{T,S}

Deduct and allocate the partitioned structures of a NLPModel using partitioned SR1 Hessian approximation.
`PSR1NLPModel` has fields:
...
"""
mutable struct PSR1NLPModel{G, P, T, S, M <: AbstractNLPModel{T, Vector{T}}, Meta <: AbstractNLPModelMeta{T, S},} <: AbstractPQNNLPModel{T,S}
  model::M
  meta::Meta
...
function PSR1NLPModel(nlp::SupportedNLPModel; type::DataType=Float64)
  n = nlp.meta.nvar
  ex = get_expression_tree(nlp)

  (n, N, vec_elt_fun, M, vec_elt_complete_expr_tree, element_expr_tree_table, index_element_tree, vec_compiled_element_gradients, x, op, fx, name) = partitioned_structure(ex, n; type, name=:psr1)
  P = typeof(op)
  ...
  counters = NLPModels.Counters()
  pvqnlp = PSR1NLPModel{ExpressionTreeForge.Complete_expr_tree, P, type, S, Model, Meta}(nlp, meta, counters, n, N, vec_elt_fun, M, vec_elt_complete_expr_tree, element_expr_tree_table, index_element_tree, vec_compiled_element_gradients, op, fx, name)
  return pvqnlp
end
