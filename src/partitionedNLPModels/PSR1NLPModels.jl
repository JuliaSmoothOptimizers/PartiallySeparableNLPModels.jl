...
"""
    PSR1NLPModel{G, T, S, M <: AbstractNLPModel{T, S}, Meta <: AbstractNLPModelMeta{T, S},} <: AbstractPQNNLPModel{T,S}
...
function PSR1NLPModel(nlp::SupportedNLPModel; type::Type{T} = eltype(nlp.meta.x0), merging::Bool = true, kwargs...) where T
  n = nlp.meta.nvar
  ...
  (
    ...
    fx,
    name,
  ) = partitioned_structure(nlp, ex, n; type, name = :psr1, merging, kwargs...)
  ...
  counters = NLPModels.Counters()
  pvqnlp = PSR1NLPModel{ExpressionTreeForge.Complete_expr_tree, type, S, Model, Meta, OB, GB}(
    nlp,
    meta,
    counters,
    n,
