...
mutable struct PLSENLPModel{
  ...
  op::PartitionedStructures.Elemental_plo{T} # partitioned quasi-Newton approximation

  fx::T
  name::Symbol
end

function PLSENLPModel(nlp::SupportedNLPModel; type::Type{T} = eltype(nlp.meta.x0), merging::Bool = true, kwargs...) where T
  n = nlp.meta.nvar
  ...
  (
    ...
    fx,
    name,
  ) = partitioned_structure(nlp, ex, n; type, name = :plse, merging, kwargs...)
  ...
  counters = NLPModels.Counters()
  pvqnlp = PLSENLPModel{ExpressionTreeForge.Complete_expr_tree, type, S, Model, Meta, OB, GB}(
    nlp,
    meta,
    counters,
    n,
