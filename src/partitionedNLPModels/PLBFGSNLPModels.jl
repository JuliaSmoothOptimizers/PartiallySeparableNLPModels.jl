...
function PLBFGSNLPModel(nlp::SupportedNLPModel; type::Type{T} = eltype(nlp.meta.x0), merging::Bool = true, kwargs...) where T
  n = nlp.meta.nvar
  ...
  (
    ...
    fx,
    name,
  ) = partitioned_structure(nlp, ex, n; type, name = :plbfgs, merging, kwargs...)
  ...
  counters = NLPModels.Counters()
  pvqnlp = PLBFGSNLPModel{ExpressionTreeForge.Complete_expr_tree, type, S, Model, Meta, OB, GB}(
    nlp,
    meta,
    counters,
    n,
