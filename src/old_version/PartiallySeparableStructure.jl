...
@inline get_index_element_tree_position(elmt_fun::element_function) =
  elmt_fun.index_element_tree_position
@inline get_fun_from_elmt_fun(elmt_fun::element_function, diff_elmt_fun::Vector{T}) where {T} =
  diff_elmt_fun[elmt_fun.index_fun]
...
@inline get_obj_pre_compiled_trees(sps::SPS{T, N}) where {T} where {N <: Number} =
  sps.obj_pre_compiled_trees
@inline get_fun_from_elmt_fun(elmt_fun::element_function, sps::SPS{T}) where {T} =
  sps.different_element_tree[elmt_fun.index_fun]
@inline get_var_from_elmt_fun(elmt_fun::element_function, sps::SPS{T}) where {T} =
  sps.x_views[get_index_fun(elmt_fun)][get_index_element_tree_position(elmt_fun)]
...
@inline set_v_sps!(sps::SPS{T, N}, vector::AbstractVector{N}) where {T} where {N <: Number} =
  sps.v .= vector
...
"""
deduct_partially_separable_structure(a::Any, n::Int, type = Float64::DataType) =
  _deduct_partially_separable_structure(a, n, type)
...
function _deduct_partially_separable_structure(tree::T, n::Int, type = Float64::DataType) where {T}
  ...
  (different_calculus_expr_tree, different_calculus_tree_index) =
    distinct_element_expr_tree(elmt_fun, elmt_var_i)
  ...
  different_calculus_complete_trees =
    map(tree -> CalculusTreeTools.cast_type_of_constant(tree, type), temp_complete_trees)
  ...
  for i = 1:m_i
    ...
    undefined_index = 0
    structure[i] = element_function(
      index_distinct_element_tree,
      undefined_index,
      type_i[index_distinct_element_tree],
      elmt_var_i[i],
  ...
  distinct_casted_expr_tree =
    map(tree -> CalculusTreeTools.cast_type_of_constant(tree, type), different_calculus_expr_tree)
  ...
  sps = SPS{CalculusTreeTools.complete_expr_tree, type}(
    structure,
    different_calculus_complete_trees,
    index_element_tree,
    related_vars,
...
evaluate_function(sps::SPS{T, N}, x::Vector{N}) where {T} where {N <: Number} =
  CalculusTreeTools.algo_expr_tree.eval_function_wrapper(get_obj_Expr(sps), x)
...
"""
function create_pre_compiled_tree(
  different_calculus_expr_tree::Vector{CalculusTreeTools.t_expr_tree},
  x_views::Vector{Vector{SubArray{N, 1, Array{N, 1}, Tuple{Array{Int64, 1}}, false}}},
) where {N <: Number}
  n = length(different_calculus_expr_tree)
  n == length(x_views) ||
    error("error in  create_pre_compiled_tree, distinct element function does not match x_views")
  res = Vector{CalculusTreeTools.pre_n_compiled_tree{N}}(undef, n)
  for i = 1:n  # for each tree create a pre-compiled tree according to the correspondinf x_views wich represent the number of evaluation
    res[i] =
      CalculusTreeTools.create_pre_n_compiled_tree(different_calculus_expr_tree[i], x_views[i])
  end
  return res
end
...
"""
function evaluate_obj_pre_compiled(
  sps::SPS{T, Y},
  x::AbstractVector{Y},
  ...
  for i = 1:n # we evaluate each pre compiled element tree. No argument x because the variables of the tree is linked with the vector x of sps.
    @inbounds res_distrinct_elmt_fun[i] =
      CalculusTreeTools.evaluate_expr_tree_multiple_points(different_element_tree[i])
  end
...
"""
construct_set_index_element_view!(
  structure::Vector{element_function},
  different_element_tree::Vector{T},
) where {T} =
  set_index_element_view!(structure, get_elmnt_fun_index_view(structure, different_element_tree))

function set_index_element_view!(
  structure::Vector{element_function},
  vector_index_view::Vector{Int},
)
...
"""
function get_elmnt_fun_index_view(
  structure::Vector{element_function},
  different_element_tree::Vector{T},
) where {T}
...
"""
function construct_views(
  x::Vector{N},
  related_vars::Vector{Vector{Vector{Int}}},
) where {N <: Number}
  nb_elmt_fun = length(related_vars)
  res =
    Vector{Vector{SubArray{N, 1, Array{N, 1}, Tuple{Array{Int64, 1}}, false}}}(undef, nb_elmt_fun)
  for i = 1:nb_elmt_fun
...
"""
function get_index_deleted(
  all_element_tree::Vector{T},
  tree::T,
  current_number_var::Int,
  current_eval_tree::Float64,
  vector_number_var::Vector{Int},
  vector_eval_elmt_tree::Vector{Float64},
) where {T}
  res = Vector{Int}(undef, 0)
  for i = 1:length(all_element_tree)
    (current_eval_tree == vector_eval_elmt_tree[i]) &&
      (vector_number_var[i] == current_number_var) &&
...
"""
eval_ni_ones(elmt_fun::T, n::Int) where {T} =
  CalculusTreeTools.evaluate_expr_tree(elmt_fun, ones(Float64, n))::Float64
function get_different_CalculusTree(
  all_elemt_fun::Vector{T},
  element_var::Vector{Vector{Int}},
) where {T}
  work_elmt_fun = copy(all_elemt_fun)
  work_number_var_vector = map(element_vars -> length(element_vars), element_var)::Vector{Int}
  work_elmt_fun_eval_to_one =
    map(eval_ni_ones, work_elmt_fun, work_number_var_vector)::Vector{Float64}
  ...
  while isempty(work_elmt_fun) == false
    ...
    current_tree_indexes = get_index_deleted(
      work_elmt_fun,
      current_tree,
      current_number_var,
      cureent_eval,
      work_number_var_vector,
      work_elmt_fun_eval_to_one,
    )
...
"""
function get_related_var(
  v_element_function::Vector{element_function},
  index_elmt_tree::Vector{Vector{Int}},
)
  new_needed_var(index) = Vector{Int}(v_element_function[index].used_variable)
  new_view_vars(index_single_elmt_tree) =
    Vector{Vector{Int}}((index -> new_needed_var(index)).(index_single_elmt_tree))
  related_vars = Vector{Vector{Vector{Int}}}(
    (index_single_elmt_tree -> new_view_vars(index_single_elmt_tree)).(index_elmt_tree),
  )
  return related_vars
end
...
"""
function get_related_function(
  v_element_function::Vector{element_function},
  diff_element_tree::Vector{T},
) where {T}
...
"""
@inline evaluate_SPS(sps::SPS{T, Y}, x::AbstractVector{Y}) where {T} where {Y <: Number} =
  evaluate_function(sps, x)
@inline evaluate_SPS(sps::SPS{T, Y}) where {T} where {Y <: Number} =
  (x::AbstractVector{Y} -> evaluate_SPS(sps, x))
...
function evaluate_SPS_gradient(sps::SPS{T, Y}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  create_element_gradient = (
    y::element_function ->
      element_gradient{Y}(Vector{typeof(x[1])}(zeros(Y, length(y.used_variable))))
  )
...
Noted that we use the previously compiled GradientTape in element_gradient! that use ReverseDiff.
"""
function evaluate_SPS_gradient!(
  sps::SPS{T},
  x::AbstractVector{Y},
  g::grad_vector{Y},
) where {T} where {Y <: Number}
  ...
  for i = 1:l_elmt_fun
    ...
    type_elmt_fun = get_type(elmt_fun)
    used_var = get_used_variable(elmt_fun)
    if isempty(used_var) == false && !CalculusTreeTools.is_constant(type_elmt_fun) #fonction element ayant au moins une variable
      index_fun = get_index_fun(elmt_fun)
      index_position_x = get_index_element_tree_position(elmt_fun)
      element_gradient!(
        sps.compiled_gradients[index_fun],
        x_views[index_fun][index_position_x],
        g.arr[i],
      )
    end
  end
...
Use of ReverseDiff
"""
function element_gradient!(
  compiled_tape::ReverseDiff.CompiledTape,
  x::AbstractVector{T},
  g::element_gradient{T},
) where {T <: Number}
...
Hv(sps::SPS{T, Y}, x::AbstractVector{Y}, v::AbstractVector{Y}) where {T} where {Y <: Number} = begin
  hv = similar(x)
  Hv!(hv, sps, x, v)
  return hv
end
function Hv!(
  ...
  f = (
    y::element_function -> element_gradient{typeof(x[1])}(
  ...
  for i = 1:l_elmt_fun
    if isempty(structure[i].used_variable) == false  #fonction element ayant au moins une variable
      ...
      Hv_elem!(
        partitionned_hv.arr[i],
        diff_element_tree[index_fun],
        x_views[index_fun][index_position_x],
        v_views[index_fun][index_position_x],
      )
    else
...
@inline SPS_ReverseDiff_grad(tree, x::AbstractVector{Y}) where {Y <: Number} = begin
  g = similar(x)
  SPS_ReverseDiff_grad!(g, tree, x)
end
@inline SPS_ReverseDiff_grad!(g::AbstractVector{Y}, tree, x) where {Y <: Number} =
  ReverseDiff.gradient!(g, CalculusTreeTools.evaluate_expr_tree(tree), x)

@inline ∇²fv(tree, x, v) = ForwardDiff.gradient(x -> dot(SPS_ReverseDiff_grad(tree, x), v), x)
@inline ∇²fv!(tree, x, v, hv) = (hv .= ∇²fv(tree, x, v))
@inline Hv_elem!(
  elemental_hv::element_gradient,
  tree::T,
  x::AbstractVector{Y},
  v::AbstractVector{Y},
) where {Y <: Number} where {T} = ∇²fv!(tree, x, v, elemental_hv.g_i)
...
"""
function grad_ni_to_n!(
  g::element_gradient{Y},
  used_var::Vector{Int},
  g_res::AbstractVector{Y},
) where {Y <: Number}
  for i = 1:length(g.g_i)
...
function evaluate_hessian(sps::SPS{T}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  ...
  for i = 1:l_elmt_fun
    elmt_hess[i] = evaluate_element_hessian(
      sps.structure[i],
      Array(view(x, sps.structure[i].used_variable)),
      sps,
    )::Tuple{Vector{Int}, Vector{Int}, Vector{Y}}
  end
  ...
  G = sparse(
    vcat(row...)::Vector{Int},
    vcat(column...)::Vector{Int},
    vcat(values...)::Vector{Y},
  )::SparseMatrixCSC{Y, Int}
  return G
end
...
"""
function evaluate_element_hessian(
  elmt_fun::element_function,
  x::AbstractVector{Y},
  sps::SPS{T},
) where {T} where {Y <: Number}
  ...
  if !CalculusTreeTools.is_linear(elmt_fun.type) && !CalculusTreeTools.is_constant(elmt_fun.type)
    temp = ForwardDiff.hessian(
      CalculusTreeTools.evaluate_expr_tree(get_fun_from_elmt_fun(elmt_fun, sps)),
      x,
    )::Array{Y, 2}
...
function struct_hessian(sps::SPS{T}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  l_elmt_fun = length(sps.structure)
  f = (
    elm_fun::element_function -> element_hessian{Y}(
      zeros(Y, length(elm_fun.used_variable), length(elm_fun.used_variable))::Array{Y, 2},
    )
  )
  t = f.(sps.structure)::Vector{element_hessian{Y}}
  temp = Hess_matrix{Y}(t)
  for i = 1:l_elmt_fun # a voir si je laisse le array(view()) la ou non
    # if sps.structure[i].type != implementation_type_expr.constant
    if !CalculusTreeTools.is_linear(sps.structure[i].type) &&
       !CalculusTreeTools.is_constant(sps.structure[i].type)
      @inbounds ForwardDiff.hessian!(
        temp.arr[i].elmt_hess,
        CalculusTreeTools.evaluate_expr_tree(get_fun_from_elmt_fun(sps.structure[i], sps)),
        view(x, sps.structure[i].used_variable),
      )
    end
  end
  return temp
...
"""
function struct_hessian!(
  sps::SPS{T},
  x::AbstractVector{Y},
  H::Hess_matrix{Y},
) where {Y <: Number} where {T}
  ...
  for i in sps.structure
    (ForwardDiff.hessian!(
      H.arr[i.index].elmt_hess::Array{Y, 2},
      CalculusTreeTools.evaluate_expr_tree(get_fun_from_elmt_fun(i, sps)::T),
      view(x, i.used_variable::Vector{Int}),
    ))
  end
end
...
function id_hessian!(sps::SPS{T}, H::Hess_matrix{Y}) where {Y <: Number} where {T}
  for i = 1:length(sps.structure)
    nᵢ = length(sps.structure[i].used_variable)
    H.arr[sps.structure[i].index].elmt_hess[:] = Matrix{Y}(I, nᵢ, nᵢ)
  end
end

construct_full_Hessian(sps::SPS{T, Y}, H::Hess_matrix{Y}) where {Y <: Number} where {T} =
  Array(construct_Sparse_Hessian(sps, H))
...
function construct_Sparse_Hessian(sps::SPS{T}, H::Hess_matrix{Y}) where {Y <: Number} where {T}
  mapreduce(
    elt_fun::element_function ->
      CalculusTreeTools.get_Ui(get_used_variable(elt_fun), sps.n_var)' *
      sparse(H.arr[elt_fun.index].elmt_hess) *
      CalculusTreeTools.get_Ui(get_used_variable(elt_fun), sps.n_var),
    +,
    sps.structure,
  )
...
function product_matrix_sps!(
  ...
  for i = 1:l_elmt_fun
    @fastmath @inbounds temp =
      B.arr[i].elmt_hess::Array{Y, 2} *
      sps.x_views[sps.structure[i].index_fun][sps.structure[i].index_element_tree_position]::SubArray{
        Y,
        1,
        Array{Y, 1},
        Tuple{Array{Int64, 1}},
...
# end
@inline f_inter!(
  res::AbstractVector{Z},
  indices::AbstractVector{Int},
  values::AbstractVector{Z},
) where {Z <: Number} = begin
...
function Hv_only_product!(
  ...
  for elmt_fun in structure
    ...
    @inbounds view(Bx, get_used_variable(elmt_fun)) .+=
      (B.arr[index_elmt_fun].elmt_hess * x_views[index_calculus_tree][index_position])
  end
end
...
"""
function product_vector_sps(
  sps::SPS{T},
  g::grad_vector{Y},
  x::Vector{Z},
) where {T} where {Y <: Number} where {Z <: Number}
  l_elmt_fun = length(sps.structure)
...
hess_full(sps::SPS{T}, x::AbstractVector{Y}) where {T} where {Y <: Number} =
  construct_full_Hessian(hess(sps, x))

function hess(sps::SPS{T}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  define_element_hessian = (
    elm_fun::element_function ->
      element_hessian{Y}(zeros(Y, length(elm_fun.used_variable), length(elm_fun.used_variable)))
  )
...
function hess!(
  partitionned_hess::Hess_matrix{Y},
  sps::SPS{T},
  x::AbstractVector{Y},
) where {T} where {Y <: Number}
  structure = get_structure(sps)
...
function elemental_hess!(
  h::element_hessian{Y},
  compiled_hessian::ReverseDiff.HessianTape,
  x::AbstractVector{Y},
) where {Y <: Number}
...
"""
function evaluate_SPS_gradient2!(
  sps::SPS{T},
  x::AbstractVector{Y},
  g::grad_vector{Y},
) where {T} where {Y <: Number}
  ...
  for i = 1:l_elmt_fun
    if isempty(sps.structure[i].used_variable) == false  #fonction element ayant au moins une variable
      element_gradient2!(
        get_fun_from_elmt_fun(sps.structure[i], sps),
        view(x, sps.structure[i].used_variable),
        g.arr[i],
      )
    end
...
This was the previous version using ForwardDiff. The actual version using ReverseDiff is more efficient.
"""
function element_gradient2!(
  expr_tree::Y,
  x::AbstractVector{T},
  g::element_gradient{T},
) where {T <: Number} where {Y}
...
function evaluate_SPS2(sps::SPS{T, Y}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  ...
  x_views = get_x_views(sps)
  (length(diff_calculus_tree) == length(related_vars) && length(related_vars) == length(x_views)) ||
    error("mismatch evaluate SPS")
  ...
  for i = 1:nb_elem_fun
    ...
    res[i] =
      sum(CalculusTreeTools.evaluate_expr_tree_multiple_points(calculus_tree::T, selected_vars))
  end
  return sum(res)
end
...
Hv2(sps::SPS{T, Y}, x::AbstractVector{Y}, v::AbstractVector{Y}) where {T} where {Y <: Number} =
  begin
    hv = similar(x)
    Hv2!(hv, sps, x, v)
    return hv
...
@inline SPS_ReverseDiff_grad2(tree, x::AbstractVector{Y}) where {Y <: Number} = begin
  g = similar(x)
  SPS_ReverseDiff_grad2!(g, tree, x)
end
@inline SPS_ReverseDiff_grad2!(g::AbstractVector{Y}, tree, x) where {Y <: Number} =
  ReverseDiff.gradient!(g, CalculusTreeTools.evaluate_expr_tree(tree), x)
...
@inline Hv_elem2!(
  ...
  x::AbstractVector{Y},
  v::AbstractVector{Y},
) where {Y <: Number} where {T} = ∇²fv2!(tree, x, v, elemental_hv.g_i)
