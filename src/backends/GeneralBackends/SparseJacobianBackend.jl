...
function set_pv_from_sparse_jacobian!(pv::PartitionedVector{T}, sparse_jacobian::Vector{T}) where T
  cpt = 1
...
function partitioned_gradient!(backend::SparseJacobianMoiModelBackend{T},
...
function objective(backend::SparseJacobianMoiModelBackend{T},
  x::PartitionedVector{T},
  ) where T
  PartitionedVectors.build!(x)
  PartitionedVectors.set!(backend.translated_x, x.epv.v)
