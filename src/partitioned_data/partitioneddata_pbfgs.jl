...
update_nlp!(
  pd_pbfgs::PartitionedData_TR_PBFGS{G, T},
  s::Vector{T};
  kwargs...,
) where {G, T <: Number} = update_PBFGS!(pd_pbfgs, s; kwargs...)
update_nlp!(
  pd_pbfgs::PartitionedData_TR_PBFGS{G, T},
  x::Vector{T},
  s::Vector{T};
  kwargs...,
) where {G, T <: Number} = update_PBFGS!(pd_pbfgs, x, s; kwargs...)
...
update_PBFGS(
  ...
  update_PBFGS!(pd_pbfgs, x, s; kwargs...)
  return Matrix(get_pB(pd_pbfgs))
end
...
"""
function build_PartitionedData_TR_PBFGS(
  tree::G,
  n::Int;
  ...
  fx = -1
  pd_pbfgs = PartitionedData_TR_PBFGS{CalculusTreeTools.complete_expr_tree, T}(
    n,
    N,
    vec_elt_fun,
    M,
