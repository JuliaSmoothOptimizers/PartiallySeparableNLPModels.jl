...
small_elements(x; n = length(x)) = sum(x[1]*x[i] for i in 1:n) + sum(x[n]*x[i] for i in 1:n)
...
function large_elements(x; n=length(x), div=Int(floor(sqrt(n))))
  ...
  sum( sub((i-1)*f+1:(i+2)*f)/(x[i]^2) for i in 1:div-3) + sum( sub((i-1)*f+5:(i+4)*f+5)/(x[n-i]^2) for i in 1:div-5) + (x[1]*x[n]^3)^2
end
